...
public class RepositoryRequestDialog extends AbstractKieRequestDialog {

	IKieOrganization organization;
	...
	public RepositoryRequestDialog(Shell shell, IKieOrganization organization) {
		super(shell, "Repository", new IKieRequestValidator() {
			...
			public String isValid(JsonObject object) {
				...
				if (name!=null && !name.isEmpty()) {
					try {
						for (IKieRepository rep : organization.getRepositories()) {
	...
	protected void createFields(Composite composite) {
		...
		userName = new KieRequestDialogTextField(composite, "Username:", "", result, "userName");
		password = new KieRequestDialogPasswordField(composite, "Password:", "", result, "password");
		requestType = new KieRequestDialogSelectionField(composite, "Request Type:",
				...
				SWT.RADIO, result, "requestType"
		);
		// "requestType" is a required field, initialize to "new"
		result.set("requestType", "new");
		
		gitURL = new KieRequestDialogTextField(composite, "URL of a Repository to clone:", "", result, "gitURL");
	...
	public JsonObject getResult() {
		...
		result.set("organizationalUnitName", organization.getName());
